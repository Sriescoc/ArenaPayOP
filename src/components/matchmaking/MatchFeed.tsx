...
import { Swords, Clock, ArrowDownUp } from 'lucide-react';
...
export function MatchFeed({ gameId, modeId, onChallenge }: MatchFeedProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    // Because we need dynamic sorting that might conflict with Firestore indexes if not created,
    // we fetch filtered by game/mode/status and sort locally.
    const q = query(
  ...
  if (loading) {
    return <div className="text-center py-10 text-slate-500 animate-pulse">Cargando tablero...</div>;
  }
    ...
    <div className="space-y-4">
      {requests.length > 0 && (
        <div className="flex justify-end mb-2">
          <button 
            ...
            className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-widest hover:text-white transition-colors bg-surface-container-low px-3 py-1.5 rounded-lg border border-white/5"
          >
      ...
      {sortedRequests.length === 0 ? (
        <div className="glass-card rounded-2xl p-8 text-center border border-white/5">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Sin Retadores</h3>
          <p className="text-slate-400 text-sm max-w-md mx-auto">No hay nadie buscando partida en este modo ahora mismo. ¡Crea tú la primera solicitud!</p>
        </div>
      ) : (
