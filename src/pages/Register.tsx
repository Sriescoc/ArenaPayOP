...
export default function Register() {
  ...
  const handleCompleteProfile = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Check if RUT already exists using the dedicated ruts collection
      const rutDocRef = doc(db, 'ruts', formData.rut);
      const rutDoc = await getDoc(rutDocRef);
      
      if (rutDoc.exists()) {
        setError('Este RUT ya está asociado a otra cuenta.');
        setLoading(false);
        return;
      }

      // Lock the RUT for this user
      await setDoc(rutDocRef, { uid: auth.currentUser.uid });
