...
export default function Profile() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'idFront' | 'idBack' | 'selfie') => {
    ...
    if (file.size > 800 * 1024) { // 800KB limit for Firestore
      alert("La imagen es muy grande. Por favor sube una de menos de 800KB.");
      return;
    }
    ...
    reader.onloadend = () => {
      setKycFiles(prev => ({ ...prev, [type]: reader.result as string }));
      setUploading(null);
    };
  ...
  const handleSimulateKYC = async () => {
    ...
    if (!kycFiles.idFront || !kycFiles.idBack || !kycFiles.selfie) {
      alert("Por favor sube todas las fotos requeridas (Carnet Frontal, Trasero y Selfie).");
      return;
    }
    ...
    try {
      ...
      await updateDoc(docRef, { 
        kycStatus: 'pending',
        idFrontUrl: kycFiles.idFront,
        idBackUrl: kycFiles.idBack,
        selfieUrl: kycFiles.selfie
      });
      ...
      alert("Error al subir documentos. Intenta con imágenes más pequeñas.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="bg-[#0a0e17] rounded-xl p-4 border border-[#1f2937] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1 font-medium">
                    {kycStatus === 'verified' ? 'Tu identidad ha sido verificada. Puedes retirar fondos.' :
                     ...
                     'Sube las fotos requeridas para verificar tu identidad y habilitar retiros.'}
                  </p>
                  ...
                  {kycStatus === 'unverified' && (
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Frontal Carnet</p>
                        <label className={`relative block aspect-video rounded-xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${kycFiles.idFront ? 'border-brand-primary bg-brand-primary/5' : 'border-navy-700 hover:border-brand-primary/50'}`}>
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, 'idFront')} />
                          {kycFiles.idFront ? (
                            <img src={kycFiles.idFront} className="w-full h-full object-cover" alt="Front" />
                          ) : (
                      ...
                      {/* ID Back */}
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Trasero Carnet</p>
                        <label className={`relative block aspect-video rounded-xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${kycFiles.idBack ? 'border-brand-primary bg-brand-primary/5' : 'border-navy-700 hover:border-brand-primary/50'}`}>
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, 'idBack')} />
                          {kycFiles.idBack ? (
                            <img src={kycFiles.idBack} className="w-full h-full object-cover" alt="Back" />
                          ) : (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500">
                              <Upload className="w-5 h-5 mb-1" />
                              <span className="text-[10px] font-bold uppercase tracking-wider">Subir</span>
                      ...
                      {/* Selfie */}
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Selfie con Carnet</p>
                        <label className={`relative block aspect-video rounded-xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${kycFiles.selfie ? 'border-brand-primary bg-brand-primary/5' : 'border-navy-700 hover:border-brand-primary/50'}`}>
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, 'selfie')} />
                          {kycFiles.selfie ? (
                            <img src={kycFiles.selfie} className="w-full h-full object-cover" alt="Selfie" />
                          ) : (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500">
                              <Upload className="w-5 h-5 mb-1" />
                              <span className="text-[10px] font-bold uppercase tracking-wider">Subir</span>
              ...
              {kycStatus === 'unverified' && (
                <button 
                  ...
                  className="bg-brand-primary hover:bg-brand-secondary text-black font-black uppercase tracking-wider py-3 px-6 rounded-xl text-xs transition-all flex items-center justify-center gap-2 whitespace-nowrap shadow-[0_0_20px_rgba(0,255,102,0.2)] disabled:opacity-50 disabled:grayscale sm:self-start mt-4"
                >
                  <Upload className="w-4 h-4" /> Verificar Identidad
                </button>
              )}
            </div>
          </div>
